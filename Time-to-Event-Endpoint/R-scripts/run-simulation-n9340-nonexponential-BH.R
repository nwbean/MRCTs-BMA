...
source("../R-scripts/full-simulation-function-nonexponential-BH.R")



### Load inputs for simulation functions
input.file <- paste("../cluster-input/input_mat_nonexpo_BH_n9340.csv")
...
pw.times <- c(0, 2, 3.75)                                 # left bounds of intervals for piecewise baseline hazards
pw.haz <- c(.02, .035, .055)                              # underlying piecewise baseline hazards
...
sim.results <- run.sims( S = S, T0 = T0, n.i = n.i, reg.names = reg.names, K = K, max.time = max.time,
                         ...
                         pw.times = pw.times, pw.haz = pw.haz, reg.te = reg.te, num.covs = num.covs,
