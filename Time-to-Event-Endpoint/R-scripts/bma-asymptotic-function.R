...
# Sig.l: prior covariance vector for theta_l
...
bma.asymptotic <- function( dat, W.mat, int.cuts, mu0, Sig0, eta0, phi0, mod.mat,
  ...
  # Also calculate ind.sum.mat, where ind.sum.mat_ik = sum_(j=1)^n.i (d_ijk * nu_ij * z_ij)
  ...
  # Calculate delta_ijk * (y_ij - m_k-1) + sum_(g=k+1)^K( delta_ijg * (m_k - m_k-1) )
  # for all subjects in region i for each time interval k. Save (n.i x K) matrix for each region
  # in a list. These summands are later used to construct phi.tilde.ik.
  ...
  for(l in 1:L){
    ...
    # Obtain Pr(|1 - exp{gamma_(l,t) - gamma_(-i)}| < epsilon.star|D,M_l) conditional on M_l,
    # l=1,...,L
    ...
    for(i in 1:S){
      ...
      # Calculate difference |1 - exp{gamma_(l,t) - gamma_(-i)}| for each region
      consis.diff.loo[,i] <- abs( 1 - exp(gamma.ti.samp - gamma.glob.no.i) )
      
    }
    
    # Calculate probability that difference |1 - exp{gamma_(l,t) - gamma_(-i)}| is less than
    # epsilon.star for each value of epsilon.star
    for(q in 1:length(epsilon.star)){
      loc.consis.loo.l[[q]][l,] <- colMeans( consis.diff.loo < epsilon.star[q] )
    }
    
    
    # Obtain pairwise consistency probabilities Pr(|1 - exp{gamma_i - gamma_j}| < epsilon_star|D,M_l)
    ...
    # Pr(|1 - exp{gamma_i - gamma_j}| > epsilon_star|D,M_l).
  ...
  # with beta.star probability. If Pr(|1 - exp{gamma_i - gamma_j}| > epsilon.star| D) >= beta.star
