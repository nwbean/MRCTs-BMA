...
source("../R-scripts/full-simulation-function-SA-K.R")
...
## Prior elicitation
K.vec <- c(4, 8, 12, 16)                         # number of intervals for piecewise exponential model
...
epsilon.star <- hr.pred * .5        # minimal clinically important regional difference, used in most measures of consistency
...
sim.results <- run.sims( S = S, T0 = T0, n.i = n.i, reg.names = reg.names, K.vec = K.vec,
                         max.time = max.time, max.enroll.time = max.enroll.time,
                         dropout.rate = dropout.rate, base.haz = base.haz, reg.te = reg.te,
                         num.covs = num.covs, cov.effects = cov.effects, cov.names = cov.names,
                         cov.props = cov.props, cov.means = cov.means, cov.sds = cov.sds,
                         mod.priors = mod.priors, mu0 = mu0, Sig0 = Sig0, gamma0 = gamma0,
                         n.draws = n.draws, epsilon.star = epsilon.star, beta.star = beta.star,
                         pi0 = pi0, sd.CPHM1 = sd.CPHM1, sd.CPHM2 = sd.CPHM2, nsims = nsims,
                         print.iters = TRUE )
