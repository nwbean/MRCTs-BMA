...
# alpha0: vector of alpha0 values used in prior model probabilities
...
run.sims <- function( S, T0, n.i, reg.names, K, max.time, max.enroll.time, dropout.rate,
                      ...
                      cov.sds, alpha0, mu0, Sig0, eta0, phi0, gamma0, n.draws, epsilon.star,
  ...
  num.alpha0 <- length(alpha0)
  BMA.gte.less.gamma0 <- matrix( 0, nrow = nsims, ncol = num.alpha0)    # Pr(gamma_G < gamma0|D)
  BMA.rte.mean <- list(num.alpha0)           # list to store regional treatment effect means
  BMA.rte.less.gamma0 <- list(num.alpha0)    # list to store Pr(gamma_i < gamma0|D), i=1,...,S
  for(q in 1:num.alpha0){
  ...
  for(j in 1:nsims){
    ...
    for(q in 1:num.alpha0){
      mod.priors <- construct_modPriors(S, T0, alpha = alpha0[q])     # prior model probabilities
  ...
  reject.proportions <- matrix( 0, nrow = 2 + num.alpha0, ncol = S+1 )
  rownames(reject.proportions) <- c("CPHM", "BHM", paste("BMA: alpha0=", alpha0, sep = ""))
  ...
  reject.proportions[3:(2+num.alpha0),1] <- colMeans(BMA.reject.glob)
  ...
  BMA.diff.rte.mat <- list(num.alpha0)
  for(q in 1:num.alpha0){
  ...
  for(i in 1:S){
    ...
    for(q in 1:num.alpha0){                                  # (estimate - true effect) for BMA
  ...
  rownames(reg.trtmt.bias) <- c("CPHM", "BHM", paste("BMA: alpha0=", alpha0, sep = ""))
  reg.trtmt.mse <- rbind( colMeans(CPHM.diff.rte.mat^2), colMeans(BHM.diff.rte.mat^2),
                          BMA.mse.rte )
  colnames(reg.trtmt.mse) <- reg.names
  rownames(reg.trtmt.mse) <- c("CPHM", "BHM", paste("BMA: alpha0=", alpha0, sep = ""))
