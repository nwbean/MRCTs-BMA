...
source("../R-scripts/full-simulation-function-SA-mu0-Sig0.R")
...
hr.pred.vals <- c(.7, 1.05, 1.3, 1.5)            # predictions of true hazard ratio
trt.pred.vals <- log(hr.pred.vals)               # predictions of true treatment effect
...
epsilon.star <- 1.3 * .5            # minimal clinically important regional difference, used in most measures of consistency
...
sim.results <- run.sims( S = S, T0 = T0, n.i = n.i, reg.names = reg.names, K = K, max.time = max.time,
                         ...
                         cov.means = cov.means, cov.sds = cov.sds, mod.priors = mod.priors,
                         trt.pred.vals = trt.pred.vals, eta0 = eta0, phi0 = phi0, gamma0 = gamma0,
