...
#epsilon.star <- c(.01, .1, .3)      # minimal clinically important regional difference, used in most measures of consistency
epsilon.star <- c( seq(0, 0.065, by = 0.005),
                   seq(0.07, .2, by = 0.01),
                   seq(.2, .25, by = .025),
                   seq(.3, .6, by = .1) )
#                                   # range of possible MCIRDs
beta.star <- .2                     # probability cutoff for which consider two regions to be clinically different,
