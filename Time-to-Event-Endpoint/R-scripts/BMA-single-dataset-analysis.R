###############################################################################################
# Execute BMA approach for one dataset
#
...
mu0 <- c( rep(trt.pred, S),
          rep(cov.preds, length(cov.pred)) )     # mean vector for prior on treatment and covariate effects
...
# Pr(|1 - exp{gamma_i - gamma_(-i)}| < epsilon.star|D) for all values of epsilon.star
...
# Pr(|1 - exp{gamma_i - gamma_j}| < epsilon.star|D) for all values of epsilon.star
