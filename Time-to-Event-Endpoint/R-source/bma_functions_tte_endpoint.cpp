...
// [[Rcpp::export]]
Rcpp::List pairwise_consistency_l(int S, Rcpp::IntegerVector modMat_l, int n_draws,
  ...
  for(int t = 0; t < T_l; t++){
    ...
    for(int w = 0; w < T_l; w++){
      ...
      // Calculate |1 - exp{gamma_(l,t) - gamma_(l,w)}| for regions that share the t^th and
      // w^th distinct treatment effects
      arma::vec prws_abs_diff = abs(1 - exp(samp_l_t - samp_l_w));
        ...
        for(int j = i+1; j < S; j++){
          ...
          if(modMat_l(i) == t+1){
            if(modMat_l(j) == w+1){
              ...
              // Calculate Pr(|1 - exp{gamma_(l,t) - gamma_(l,w)}| < epsilon_star|D,M_l)
  ...
  for(int q = 0; q < epsilon_star.size(); q++){
    ...
    for(int i = 0; i < S; i++){
      for(int j = 0; j < S; j++){
        ...
        // Extract Pr(|1 - exp{gamma_i - gamma_j}| < epsilon_star|D, M_l) and
        // Pr(|1 - exp{gamma_i - gamma_j}| > epsilon_star|D, M_l) for all L models
        ...
        // Posterior Pr(|1 - exp{gamma_i - gamma_j}| < epsilon_star|D) and
        // posterior Pr(|1 - exp{gamma_i - gamma_j}| > epsilon_star|D) 
...
// ### Function to calculate epsilon_star-level global consistency/inconsistency probabilities
// # User input must include the following:
//     S: number of regions
    ...
    for(int l = 0; l < numMods; l++){
      ...
      for(int i = 0; i < S; i++){
        ...
        for(int j = i+1; j < S; j++){
          ...
          // If Pr(|1 - exp{gamma_i - gamma_j}| > epsilon_star | D) < beta_star, consider
