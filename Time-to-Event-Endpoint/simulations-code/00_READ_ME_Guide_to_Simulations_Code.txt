GUIDE TO SIMULATIONS CODE (MORE-DETAILED GUIDES PROVIDED IN EACH FOLDER):
...
cluster-results
   - Folders for each simulation study containing results for the global
     ...
     and MSE for region-specific treatment effects, PMPs, local constancy probabilities
     (two variations of the Japanese MHLW approach and the proposed leave-one-out absolute
   ...
   - NOTE: results cannot be uploaded to GitHub due to size. All results can be reproduced
     by rerunning the simulations using the files provided.
...
R-source
   - CPP files with Rcpp code and TXT file with JAGS scripts for Bayesian hierarchical models
