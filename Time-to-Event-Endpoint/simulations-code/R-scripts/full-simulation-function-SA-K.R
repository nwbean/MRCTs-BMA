...
# K.vec: vector of different numbers of time intervals in piecewise exponential model
...
run.sims <- function( S, T0, n.i, reg.names, K.vec, max.time, max.enroll.time, dropout.rate,
                      base.haz, reg.te, num.covs, cov.effects, cov.names, cov.props, cov.means,
                      cov.sds, mod.priors, mu0, Sig0, gamma0, n.draws, epsilon.star,
  ...
  num.K <- length(K.vec)
  BMA.gte.less.gamma0 <- matrix( 0, nrow = nsims, ncol = num.K)    # Pr(gamma_G < gamma0|D)
  BMA.rte.mean <- list(num.K)               # list to store regional treatment effect means
  BMA.rte.less.gamma0 <- list(num.K)        # list to store Pr(gamma_i < gamma0|D), i=1,...,S
  for(q in 1:num.K){
  ...
  for(j in 1:nsims){
    ...
    for(q in 1:num.K){
      ...
      int.cuts <- c( 0, quantile( dat.full$eventtime[which(dat.full$status == 1)],
                                  probs = seq(1/K.vec[q], 1, length = K.vec[q])[-K.vec[q]] ) )
      dat.full$interval <- apply( cbind(dat.full$eventtime), 1, function(x){ sum(x > int.cuts) } )
      
      # Matrices with hyperparamters for baseline hazards
      eta0 <- matrix( 0.01, nrow = S, ncol = K.vec[q] )       # shape hyperparameters
      phi0 <- matrix( 0.01, nrow = S, ncol = K.vec[q] )       # rate hyperparameters
  ...
  reject.proportions <- matrix( 0, nrow = 2 + num.K, ncol = S+1 )
  rownames(reject.proportions) <- c("CPHM", "BHM", paste("BMA: K=", K.vec, sep = ""))
  ...
  reject.proportions[3:(2+num.K),1] <- colMeans(BMA.reject.glob)
  ...
  for(i in 1:S){
    ...
    for(q in 1:num.K){                                  # (estimate - true effect) for BMA
  ...
  rownames(reg.trtmt.bias) <- c("CPHM", "BHM", paste("BMA: K=", K.vec, sep = ""))
  reg.trtmt.mse <- rbind( colMeans(CPHM.diff.rte.mat^2), colMeans(BHM.diff.rte.mat^2),
                          BMA.mse.rte )
  colnames(reg.trtmt.mse) <- reg.names
  rownames(reg.trtmt.mse) <- c("CPHM", "BHM", paste("BMA: K=", K.vec, sep = ""))
