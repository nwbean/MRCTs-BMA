...
#sim.version <- "n9340-null-half"
#sim.version <- "n9340-alt-half"
#sim.version <- "n9340-nonexponential-BH"
...
#tot.num.results <- 50                                # use for version "n9340-null-half"
#tot.num.results <- 50                                # use for version "n9340-alt-half"
#tot.num.results <- 500                               # use for version "n9340-nonexponential-BH"
...
### Assess PMPs
round( colMeans(pmp.list[[1]]), 4)     # Model 1 is correct
round( colMeans(pmp.list[[2]]), 4)     # Model 10 is correct
round( colMeans(pmp.list[[3]]), 4)     # Model 4 is correct
round( colMeans(pmp.list[[4]]), 4)     # Model 2 is correct
round( colMeans(pmp.list[[5]]), 4)     # Model 1 is correct
...
tpr.BHM <- c( rowMeans(rr.list[[1]][3,3:6]), rowMeans(rr.list[[2]][3,4:6]),
...
fpr.BHM <- c( NULL, rr.list[[2]][3,3], rowMeans(rr.list[[3]][3,3:4]),
...
grr.mat <- matrix( rbind(grr.CPHM, grr.BMA, grr.BHM), byrow = FALSE, nrow = 1 )
...
text(.5, .95, "A", cex = 2)
legend( "topright", bty = "n", legend = c("CPHM", "BMA", "BHM"),
...
legend( "topright", bty = "n", legend = c("BMA (alt. regions)", "BHM (alt. regions)"),
...
TPR.lim <- .6
tpr.mat <- matrix( rbind(tpr.CPHM, tpr.BMA, tpr.BHM), byrow = FALSE, nrow = 1 )
...
FPR.lim <- 0.14
fpr.mat <- matrix( rbind(fpr.CPHM, fpr.BMA, fpr.BHM), byrow = FALSE, nrow = 1 )
...
text(.3, .95, "A", cex = 1.2)
legend( x = 9, y = 1.01, bty = "n", legend = c("CPHM", "BMA", "BHM"), horiz = TRUE,
...
legend( x = 7.1, y = 1.22, bty = "n", legend = c("BMA", "BHM"), horiz = TRUE,
...
legend( x = 7.1, y = 1.6, bty = "n", legend = c("BMA", "BHM"), horiz = TRUE,
        fill = c("gray60", "gray90"), cex = .9 )
box()
...
##################################################################################################
# Local and pairwise consistency results for the following versions:
#   "n9340-equal-samp"
#   "n9340-null-half"
#   "n9340-alt-half"
#   "n9340-nonexponential-BH"
...
### See file "compile-global-consistency-results.R" to compile results for global consistency measure



### Compile results for local consistency measures

## Local consistency using PMDA ratio with relative risk (not recommended approach)

# Median local consistency probabilities by scenario
round( apply( lc.PMDA.RR.list[[1]], 2, median ), 3 )
round( apply( lc.PMDA.RR.list[[2]], 2, median ), 3 )
round( apply( lc.PMDA.RR.list[[3]], 2, median ), 3 )
round( apply( lc.PMDA.RR.list[[4]], 2, median ), 3 )
round( apply( lc.PMDA.RR.list[[5]], 2, median ), 3 )


## Local consistency using PMDA ratio with treatment effect (not recommended approach)

# Median local consistency probabilities by scenario
round( apply( lc.PMDA.TE.list[[1]], 2, median ), 3 )
round( apply( lc.PMDA.TE.list[[2]], 2, median ), 3 )
round( apply( lc.PMDA.TE.list[[3]], 2, median ), 3 )
round( apply( lc.PMDA.TE.list[[4]], 2, median ), 3 )
round( apply( lc.PMDA.TE.list[[5]], 2, median ), 3 )
...
##################################################################################################
# Local and pairwise consistency results for version "n9340-original"
##################################################################################################
