...
source("../R-scripts/full-simulation-function-BHM-priors.R")



### Load inputs for simulation functions
input.file <- paste("../cluster-input/input_mat_equal_samp_n9340.csv")
...
epsilon.star <- c( seq(.5, 0.7, by = 0.05),
                   seq(0.71, .76, by = 0.01), 1/hr.pred,
                   seq(.77, .95, by = 0.01),
                   seq(.955, 1, by = 0.005) )
...
dir.create( paste("../cluster-results/", sim.version, "/BHM-hier-var", sep=""), showWarnings = FALSE )
dir.create( paste("../cluster-results/", sim.version, "/gelman-rubin", sep=""), showWarnings = FALSE )
...
output.file.hv <- paste("../cluster-results/", sim.version, "/BHM-hier-var/ct_", sim.version, "-", row.id, ".csv", sep="")
output.file.gr.pe <- paste("../cluster-results/", sim.version, "/gelman-rubin/pe_", sim.version, "-", row.id, ".csv", sep="")
output.file.gr.uci <- paste("../cluster-results/", sim.version, "/gelman-rubin/uci_", sim.version, "-", row.id, ".csv", sep="")
output.file.gr.multi <- paste("../cluster-results/", sim.version, "/gelman-rubin/multi_", sim.version, "-", row.id, ".csv", sep="")
...
write.csv(sim.results$BHM.Hier.Var.Stats, output.file.hv, quote = FALSE, row.names = FALSE)
write.csv(sim.results$BHM.PSRF.Point.Ests, output.file.gr.pe, quote = FALSE, row.names = FALSE)
write.csv(sim.results$BHM.PSRF.Upper.Cred.Ints, output.file.gr.uci, quote = FALSE, row.names = FALSE)
write.csv(sim.results$BHM.Multivariate.PSRF, output.file.gr.multi, quote = FALSE, row.names = FALSE)
