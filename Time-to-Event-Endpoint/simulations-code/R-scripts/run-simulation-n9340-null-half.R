...
input.file <- paste("../cluster-input/input_mat_null_half_n9340.csv")
...
if(row.id %in% c(1:10, 41:50)){
  n.i <- floor(N * reg.allctn) +
    c(rep(1, N %% S), rep(0, S - (N %% S)))               # regional sample size
} else if(row.id %in% 11:20){
  n.i <- floor(N * reg.allctn) +
    c(N - sum(floor(N * reg.allctn)), 0, 0, 0)            # regional sample size
} else if(row.id %in% 21:30){
  n.i <- floor(N * reg.allctn) +
    c(ceiling( (N - sum(floor(N * reg.allctn)))/2 ),
      floor( (N - sum(floor(N * reg.allctn)))/2 ), 0, 0)  # regional sample size
} else if(row.id %in% 31:40){
  n.i <- floor(N * reg.allctn) +
    c(rep(1, N %% S), rep(0, S - (N %% S)))               # regional sample size
}
