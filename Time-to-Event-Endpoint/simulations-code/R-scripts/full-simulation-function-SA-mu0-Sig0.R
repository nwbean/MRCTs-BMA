...
# trt.pred.vals: vector of predictions of true treatment effect
...
run.sims <- function( S, T0, n.i, reg.names, K, max.time, max.enroll.time, dropout.rate, base.haz,
                      reg.te, num.covs, cov.effects, cov.names, cov.props, cov.means, cov.sds,
                      mod.priors, trt.pred.vals, eta0 = eta0, phi0 = phi0, gamma0, n.draws,
                      epsilon.star, beta.star, pi0, nsims, sd.CPHM1, sd.CPHM2, print.iters = FALSE ){
  ...
  num.trt.preds <- length(trt.pred.vals)
  BMA.gte.less.gamma0 <- matrix( 0, nrow = nsims, ncol = num.trt.preds)    # Pr(gamma_G < gamma0|D)
  BMA.rte.mean <- list(num.trt.preds)               # list to store regional treatment effect means
  BMA.rte.less.gamma0 <- list(num.trt.preds)        # list to store Pr(gamma_i < gamma0|D), i=1,...,S
  for(q in 1:num.trt.preds){
    BMA.rte.mean[[q]] <- matrix( 0, nrow = nsims, ncol = S)
    BMA.rte.less.gamma0[[q]] <- matrix( 0, nrow = nsims, ncol = S) 
  }
  ...
  for(j in 1:nsims){
    ...
    for(q in 1:num.trt.preds){
      ...
      # Mean and covariance hyperparameters for treatment effects
      mu0 <- rep(trt.pred.vals[q], S)                          # mean vector
      Sig0 <- diag( rep((10 * abs(trt.pred.vals[q]))^2, S) )   # covariance matrix
      ...
      BMA.gte.less.gamma0[j,q] <- bma.sim.results$gte.less.gamma0    # Pr(gamma_G < gamma0|D)
      BMA.rte.mean[[q]][j,] <- bma.sim.results$rte.means    # posterior means of regional trtmt effects
      BMA.rte.less.gamma0[[q]][j,] <- bma.sim.results$rte.less.gamma0   # Pr(gamma_i < gamma0|D)
    }
  ...
  reject.proportions <- matrix( 0, nrow = 2 + num.trt.preds, ncol = S+1 )
  rownames(reject.proportions) <- c("CPHM", "BHM",
                                    paste("BMA: exp{mu0}=", round(exp(trt.pred.vals), 2), sep = ""))
  ...
  reject.proportions[2,1] <- mean(BHM.reject.glob)
  reject.proportions[3:(2+num.trt.preds),1] <- colMeans(BMA.reject.glob)
  
  ## Regional treatment effects
  reject.proportions[1, 2:(S+1)] <- colMeans(CPHM.rte.less.gamma0 > .975)
  reject.proportions[2, 2:(S+1)] <- colMeans(BHM.rte.less.gamma0 > .975)
  for(q in 1:num.trt.preds){
    reject.proportions[2+q, 2:(S+1)] <- colMeans(BMA.rte.less.gamma0[[q]] > .975)
  }
  ...
  BMA.diff.rte.mat <- list(num.trt.preds)
  for(q in 1:num.trt.preds){
    BMA.diff.rte.mat[[q]] <- matrix( 0, nrow = nsims, ncol = S )
  }
  for(i in 1:S){
    ...
    for(q in 1:num.trt.preds){                                  # (estimate - true effect) for BMA
      BMA.diff.rte.mat[[q]][,i] <- BMA.rte.mean[[q]][,i] - reg.te[i]
    }
  }
  BMA.bias.rte <- NULL
  BMA.mse.rte <- NULL
  for(q in 1:num.trt.preds){
    BMA.bias.rte <- rbind(BMA.bias.rte, colMeans(BMA.diff.rte.mat[[q]]))
    BMA.mse.rte <- rbind(BMA.mse.rte, colMeans(BMA.diff.rte.mat[[q]]^2))
  }
  reg.trtmt.bias <- rbind( colMeans(CPHM.diff.rte.mat), colMeans(BHM.diff.rte.mat),
                           BMA.bias.rte )
  colnames(reg.trtmt.bias) <- reg.names
  rownames(reg.trtmt.bias) <- c("CPHM", "BHM",
                                paste("BMA: exp{mu0}=", round(exp(trt.pred.vals), 2), sep = ""))
  reg.trtmt.mse <- rbind( colMeans(CPHM.diff.rte.mat^2), colMeans(BHM.diff.rte.mat^2),
                          BMA.mse.rte )
  colnames(reg.trtmt.mse) <- reg.names
  rownames(reg.trtmt.mse) <- c("CPHM", "BHM",
                               paste("BMA: exp{mu0}=", round(exp(trt.pred.vals), 2), sep = ""))
