...
input.file <- paste("../cluster-input/input_mat_alt_half_n9340.csv")
...
if(row.id %in% c(1:10, 41:50)){
  ...
  n.i <- floor(N * reg.allctn) +
    c(0,0, ceiling( (N - sum(floor(N * reg.allctn)))/2 ),
      floor( (N - sum(floor(N * reg.allctn)))/2 ))  # regional sample size
