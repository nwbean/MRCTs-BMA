...
run.sims <- function(N, S, T0, reg.names, reg.allctn, trtmt.allctn, trtmt.means, cntrl.means,
  ...
  ### Determine correct model (for BMA)
  mod.mat <- compile_modMat(S, T0)
  true.mod <- numeric(S)
  true.effects <- trtmt.means - cntrl.means
  for(i in 1:S){
    true.mod[i] <- which( unique(true.effects) == true.effects[i])
  }
  correct.mod.num <- 0
  for(t in 1:nrow(mod.mat)){
    correct.mod.num <- ifelse( sum( mod.mat[t,] == true.mod ) == S, t, correct.mod.num )
  }
