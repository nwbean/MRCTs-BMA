...
bma.joint.model <- function( surv.dat, cov.surv.names = NULL, int.cuts, long.dat,
  ...
  mods.fit$D.long.l <- apply(mod.mat[mods.fit[,2],], 1,
                             function(x) length(unique(x)))  # numb. distinct trt effects for long. submodels
  mods.fit$D.surv.l <- apply(mod.mat[mods.fit[,3],], 1,
  ...
  mods.fit$PMP <- 0               # place holder for posterior model probabilities
    ...
    while(stop.optimization == 0){
      ...
      if(stop.optimization == 0){
        ...
        for(j in 1:N){
          ...
          z.t.mat.ij <- matrix( z.time.mat[long.rows.ij,], nrow = length(long.rows.ij) )
  ...
  if( is.null(mod.priors) ){
    
    # Use prior model probabilities \propto exp(D.Yl * alpha0.surv + D.Xl*alpha0.long)
    mod.priors <- exp( mods.fit[,4] * a0.surv + mods.fit[,5] * a0.long ) /
      sum( exp( mods.fit[,4] * a0.surv + mods.fit[,5] * a0.long ) )
    ...
    # Use prespecified prior model probabilities
    max.log.p.D <- max(mods.fit[,6])
    ml.prop <- exp( mods.fit[,6] - max.log.p.D)
    pmp.vec <- ( ml.prop * mod.priors ) / sum( ml.prop * mod.priors )
    mods.fit[,7] <- pmp.vec
  ...
  # Obtain n.draws samples from posterior distributions of global and region-specific treatment
  # effects (mixture of normal distributions) for survival submodel, and obtain 95% central
  # credible intervals
  cred.ints.surv <- matrix(0, nrow = S + 1, ncol = 2)   # store bounds of 95% credible ints.
  rownames(cred.ints.surv) <- c("Global", paste0("Reg", 1:S))
  colnames(cred.ints.surv) <- c("LowBound", "UpBound")
  which.mods.glob <- sample(1:L.all, n.draws, pmp.vec, replace = TRUE)   # sample model indicators
  glob.surv.samp <- rnorm(n.draws, gte.surv.mean.l[which.mods.glob],
                          gte.surv.sd.l[which.mods.glob])
  cred.ints.surv[1,] <- quantile(glob.surv.samp, c(.025, .975))
  for(i in 1:S){
    which.mods.reg <- sample(1:L.all, n.draws, pmp.vec, replace = TRUE)  # sample model indicators
    reg.i.surv.samp <- rnorm(n.draws, rte.surv.mean.l.mat[which.mods.reg, i],
                             rte.surv.sd.l.mat[which.mods.reg, i])
    cred.ints.surv[i+1,] <- quantile(reg.i.surv.samp, c(.025, .975))
  }
  
  
  # Calculate 95% credible interval for association parameter(s) alpha
  cred.int.alpha <- matrix(0, nrow = num.alpha, ncol = 2)   # store bounds of 95% credible ints.
  rownames(cred.int.alpha) <- paste0("alpha", 1:num.alpha)
  colnames(cred.int.alpha) <- c("LowBound", "UpBound")
  which.mods.alpha <- sample(1:L.all, n.draws, replace = TRUE, prob = pmp.vec)
  if(num.alpha == 1){
    alpha.samp <- rnorm(n.draws, alpha.mean.l[which.mods.alpha,1],
                        alpha.sd.l[which.mods.alpha,1])
    cred.int.alpha[1,] <- quantile(alpha.samp, c(.025, .975))
  } else if(num.alpha == 2){
    alpha1.samp <- rnorm(n.draws, alpha.mean.l[which.mods.alpha,1],
                         alpha.sd.l[which.mods.alpha,1])
    alpha2.samp <- rnorm(n.draws, alpha.mean.l[which.mods.alpha,2],
                         alpha.sd.l[which.mods.alpha,2])
    cred.int.alpha[1,] <- quantile(alpha1.samp, c(.025, .975))
    cred.int.alpha[2,] <- quantile(alpha2.samp, c(.025, .975))
  }
  
  
  # Calculate posterior summary statistics for region-specific treatment effects in longitudinal
  # model (including time interaction effects)
  rte.long.mean.mat <- matrix( 0, nrow = 2 + num.knots, ncol = S )  # posterior means
  rownames(rte.long.mean.mat) <- trt.long.names
  colnames(rte.long.mean.mat) <- paste("Reg", 1:S)
  rte.long.sd.mat <- rte.long.mean.mat                              # posterior stand. devs.
  rte.long.less.gamma0.mat <- rte.long.mean.mat                     # Pr(gamma_i < gamma_0|D)
  rte.long.grtr.gamma0.mat <- rte.long.mean.mat                     # Pr(gamma_i > gamma_0|D)
  for(j in 1:(2 + num.knots)){
    rte.long.mean.mat[j,] <- rbind(pmp.vec) %*% rte.long.mean.l.list[[j]]
    rte.long.sd.mat[j,] <- rbind(pmp.vec) %*% rte.long.sd.l.list[[j]]
    rte.long.less.gamma0.mat[j,] <- rbind(pmp.vec) %*% rte.long.less.gamma0.l.list[[j]]
    rte.long.grtr.gamma0.mat[j,] <- rbind(pmp.vec) %*% rte.long.grtr.gamma0.l.list[[j]]
  }
  
  
  # Calculate Pr(gamma < gamma_0|D) and Pr(gamma > gamma_0|D) for global treatment effects
  # in longitudinal model (including time interaction effects)
  gte.long.prob.gamma0.mat <- matrix( 0, nrow = 2, ncol = 2 + num.knots )
  rownames(gte.long.prob.gamma0.mat) <- c("ProbLessThanGamma0", "ProbGreaterThanGamma0")
  colnames(gte.long.prob.gamma0.mat) <- colnames(gte.long.less.gamma0.l)
  gte.long.prob.gamma0.mat[1,] <-rbind(pmp.vec) %*% cbind(gte.long.less.gamma0.l)
  gte.long.prob.gamma0.mat[2,] <-rbind(pmp.vec) %*% cbind(gte.long.grtr.gamma0.l)
  
  
  # Calculate epsilon.star-level pairwise consistency for all values of epsilon.star (surv.)
  ...
  # Calculate epsilon.star-level global consistency/inconsistency probabilities (surv.)
  ...
  mod.mat.extend <- NULL   # create model matrix corresponding to survival partitions for all models
  for(l in 1:L.long){
    mod.mat.extend <- rbind(mod.mat.extend, mod.mat)
  }
  glob.consis.vec <- global_consistency(S = S, modMat = mod.mat.extend, pmp = pmp.vec,
  ...
  BMA.results$gte.surv.mean <- rbind(pmp.vec) %*% cbind(gte.surv.mean.l)
  BMA.results$gte.surv.sd <- rbind(pmp.vec) %*% cbind(gte.surv.sd.l)
  BMA.results$gte.surv.less.gamma0 <- rbind(pmp.vec) %*% cbind(gte.surv.less.gamma0.l)
  BMA.results$gte.long.mean <- rbind(pmp.vec) %*% cbind(gte.long.mean.l)
  BMA.results$gte.long.sd <- rbind(pmp.vec) %*% cbind(gte.long.sd.l)
  BMA.results$gte.long.probs <- gte.long.prob.gamma0.mat
  BMA.results$rte.surv.means <- rbind(pmp.vec) %*% rte.surv.mean.l.mat
  BMA.results$rte.surv.sds <- rbind(pmp.vec) %*% rte.surv.sd.l.mat
  BMA.results$rte.surv.less.gamma0 <- rbind(pmp.vec) %*% rte.surv.less.gamma0.prob.l.mat
  BMA.results$rte.long.means <- rte.long.mean.mat
  BMA.results$rte.long.sds <- rte.long.sd.mat
  BMA.results$rte.long.less.gamma0 <- rte.long.less.gamma0.mat
  BMA.results$rte.long.grtr.gamma0 <- rte.long.grtr.gamma0.mat
  BMA.results$credible.intervals.surv <- cred.ints.surv
  BMA.results$cov.surv.means <- rbind(pmp.vec) %*% covs.surv.means.l.mat
  BMA.results$cov.surv.sds <- rbind(pmp.vec) %*% covs.surv.sds.l.mat
  BMA.results$cov.long.means <- rbind(pmp.vec) %*% covs.long.means.l.mat
  BMA.results$cov.long.sds <- rbind(pmp.vec) %*% covs.long.sds.l.mat
  BMA.results$alpha.mean <- rbind(pmp.vec) %*% cbind(alpha.mean.l)
  BMA.results$alpha.sd <- rbind(pmp.vec) %*% cbind(alpha.sd.l)
  BMA.results$credible.interval.alpha <- cred.int.alpha
  ...
  BMA.results$global.consistency.surv <- glob.consis.vec
  BMA.results$pairwise.consistency.surv <- bma.prws.consis
  BMA.results$pairwise.inconsistency.surv <- bma.prws.inconsis
