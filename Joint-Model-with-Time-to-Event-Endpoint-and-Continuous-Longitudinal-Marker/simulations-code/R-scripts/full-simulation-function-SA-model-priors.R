...
source("../R-scripts/bma-joint-model-function-model-priors.R")   # function for BMA (joint model)
...
run.sims <- function(
  ...
  # (survival submodel)
  BMA.JM.gte.surv.less.gamma0 <- matrix(0, nrow = nsims,
                                        ncol = length(a0.surv))    # Pr(gamma_G < gamma0|D)
  BMA.JM.rte.surv.mean <- list(length(a0.surv))            # reg-specific treatment effect means
  BMA.JM.rte.surv.less.gamma0 <- list(length(a0.surv))     # Pr(gamma_i < gamma0|D), i=1,...,S
  for(q in 1:length(a0.surv)){
    BMA.JM.rte.surv.mean[[q]] <- matrix(0, nrow = nsims, ncol = S)
    BMA.JM.rte.surv.less.gamma0[[q]] <- matrix(0, nrow = nsims, ncol = S)
  }
  ...
  mod.priors.BMA.S <- construct_modPriors(S, T0, alpha = 0)    # prior model probabilities
  ...
  for(m in 1:nsims){
    ...
    for(q in 1:length(a0.surv)){
      BMA.JM.rte.surv.mean[[q]][m,] <-
        bma.jm.sim.results$rte.surv.means[q,]  # post. means of reg. trtmt effects
      BMA.JM.rte.surv.less.gamma0[[q]][m,] <-
        bma.jm.sim.results$rte.surv.less.gamma0[q,]  # Pr(gamma_i < gamma0|D), i=1,...,S
    }
  ...
  reject.proportions.surv <- matrix( 0, nrow = 3 + length(a0.surv), ncol = S+1 )
  rownames(reject.proportions.surv) <- c("CPHM", "BMA-S", "BHM",
                                         paste0("BMA-JM-a_s=", a0.surv, "-a_l=", a0.long))
  ...
  reject.proportions.surv[2,1] <- mean(BMA.S.reject.glob)
  reject.proportions.surv[3,1] <- mean(BHM.reject.glob)
  for(q in 1:length(a0.surv)){
    reject.proportions.surv[3+q,1] <- mean(BMA.JM.reject.glob.surv[,q]) 
  }
  
  ## Regional treatment effects (surv.)
  reject.proportions.surv[1, 2:(S+1)] <- colMeans(CPHM.rte.less.gamma0 > .975)
  reject.proportions.surv[2, 2:(S+1)] <- colMeans(BMA.S.rte.less.gamma0 > .975)
  reject.proportions.surv[3, 2:(S+1)] <- colMeans(BHM.rte.less.gamma0 > .975)
  for(q in 1:length(a0.surv)){
    reject.proportions.surv[3+q, 2:(S+1)] <- colMeans(BMA.JM.rte.surv.less.gamma0[[q]] > .975)
  }
  colnames(reject.proportions.surv) <- c("Global", reg.names)
  
  
  ### Posterior means of regional treatment effects (surv.)
  BMA.JM.rte.surv.mean.mat <- matrix(0, nrow = length(a0.surv), ncol = S)
  for(q in 1:length(a0.surv)){
    BMA.JM.rte.surv.mean.mat[q,] <- colMeans(BMA.JM.rte.surv.mean[[q]])
  }
  rte.post.means.surv <- rbind( colMeans(CPHM.rte.mean), colMeans(BMA.S.rte.mean),
                                colMeans(BHM.rte.mean), BMA.JM.rte.surv.mean.mat )
  colnames(rte.post.means.surv) <- reg.names
  rownames(rte.post.means.surv) <- c("CPHM", "BMA-S", "BHM",
                                     paste0("BMA-JM-a_s=", a0.surv, "-a_l=", a0.long))
  ...
  BMA.JM.reg.trtmt.bias.surv <- matrix( 0, nrow = length(a0.surv), ncol = S )
  BMA.JM.reg.trtmt.mse.surv <- matrix( 0, nrow = length(a0.surv), ncol = S )
  for(q in 1:length(a0.surv)){
    BMA.JM.diff.rte.surv.mat <- matrix( 0, nrow = nsims, ncol = S )
    for(i in 1:S){
      BMA.JM.diff.rte.surv.mat[,i] <-
        BMA.JM.rte.surv.mean[[q]][,i] - reg.te.surv[i]            # (estimate - true effect) for BMA-JM
    }
    BMA.JM.reg.trtmt.bias.surv[q,] <- colMeans(BMA.JM.diff.rte.surv.mat)
    BMA.JM.reg.trtmt.mse.surv[q,] <- colMeans(BMA.JM.diff.rte.surv.mat^2)
  }
  reg.trtmt.bias.surv <- rbind( colMeans(CPHM.diff.rte.mat), colMeans(BMA.S.diff.rte.mat),
                                colMeans(BHM.diff.rte.mat), BMA.JM.reg.trtmt.bias.surv)
  colnames(reg.trtmt.bias.surv) <- reg.names
  rownames(reg.trtmt.bias.surv) <- c("CPHM", "BMA-S", "BHM",
                                     paste0("BMA-JM-a_s=", a0.surv, "-a_l=", a0.long))
  reg.trtmt.mse.surv <- rbind( colMeans(CPHM.diff.rte.mat^2), colMeans(BMA.S.diff.rte.mat^2),
                               colMeans(BHM.diff.rte.mat^2), BMA.JM.reg.trtmt.mse.surv )
  colnames(reg.trtmt.mse.surv) <- reg.names
  rownames(reg.trtmt.mse.surv) <- c("CPHM", "BMA-S", "BHM",
                                    paste0("BMA-JM-a_s=", a0.surv, "-a_l=", a0.long))
