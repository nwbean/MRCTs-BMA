...
sim.version <- "equal-samp-log-gamma-re"
nsims <- 20
...
source("../R-scripts/full-simulation-function-log-gamma-re.R")
...
## Distributional values of subject-specific random intercepts (log-gamma)
REs <- "int"                     # type of random effects to include ("int" or "int-slope")
#b.mean <- 0                      # mean
#b.var <- 1.057^2                 # variance
b.shape <- 1.314376              # shape parameter of log-gamma distribution
b.rate <- 0.8581164              # rate parameter of log-gamma distribution
# Note that the shape and rate parameters of the log-gamma distribution for the random effects
# were chosen such that the moments of this distribution are approximately equal to the moments
# of the normal distribution used in all other simulations; i.e.,
# mean of log-gamma distribution = digamma(b.shape) - log(b.rate) = 0
# variance of log-gamma distribution = trigamma(b.shape) = 1.057^2
...
sim.results <- run.sims(
  ...
  b.shape = b.shape, b.rate = b.rate, alpha.vec = alpha.vec, vst.times = vst.times,
