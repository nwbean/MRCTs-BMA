...
# b.shape: shape parameter of log-gamma distribution for subject-specific random intercepts
# b.rate: rate parameter of log-gamma distribution for subject-specific random intercepts
...
run.sims <- function(
  ...
  S, T0, n.i, reg.names, pw.hazs, pw.times, reg.te.surv, sigma.e, b.shape, b.rate,
  alpha.vec, vst.times, vst.means.0, vst.means.1, max.time, dropout.rate,
  # Covariate details
  ...
  for(m in 1:nsims){
    ...
    b.mat <- cbind( log(rgamma(N, shape = b.shape, rate = b.rate)) )
