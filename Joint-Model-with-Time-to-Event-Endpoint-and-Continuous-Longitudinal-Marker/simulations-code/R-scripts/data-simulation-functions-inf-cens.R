...
#      e.lp: exp(lp) for a given linear predictor lp that does not depend on time
S.int.q.fun <- function(t.val, t.qm1, lambda.q, e.lp){
...
#      lambda.q: baseline hazard for qth interval
#      e.lp: exp(lp) for a given linear predictor lp that does not depend on time
S.int.q.inv.fun <- function(u, t.qm1, lambda.q, e.lp){
...
#      e.lp.vec: N x 1 vector of exp(lp) values for linear predictors lp that do not depend on time
sim.events <- function(N, lambda.vec, t.knots, e.lp.vec){
  ...
  for(i in 1:N){
    ...
    for(q in 1:Q){
      ...
      if(surv.q == 0){
        ...
        S.t.q <- S.int.q.fun(t.knots[q+1], t.knots[q], lambda.vec[q], e.lp.vec[i])
        S.t.qm1 <- S.int.q.fun(t.knots[q], t.knots[q], lambda.vec[q], e.lp.vec[i])
        u <- runif(1, min = S.t.q, max = S.t.qm1)
        time.vec[i] <- S.int.q.inv.fun(u, t.knots[q], lambda.vec[q], e.lp.vec[i])
...
sim.joint.data <- function( N, reg.vec, trt.vec, base.haz.vec, pw.times, W.surv, W.long,
  ...
  # N x 1 vector of exp(lp) where lp is the linear predictor that does not depend on time
  ...
  ## Identify individuals with at least one X.ijk value >= 10 (not counting baseline) and censor
  ## them at the time of the earliest occurrence, removing all later X.ijk values
  long.dat.grtr.10 <- ifelse(long.dat$X.ijk >= 10 & long.dat$time != 0, 1, 0)
  subj.inf.cens <- which( tapply(long.dat.grtr.10, long.dat$subjid, sum) > 0 )
  
  ## Create vector with values that indicate the following visit details:
  ##    0: visits for which an individual has not yet observed X.ijk >= 10
  ##    1: an individual's first visit with observed X.ijk >= 10 (last visit before withdrawal)
  ##    >1: visits following an individual's first observed X.ijk >= 10 (remove these visits)
  visit.vec <- unlist(tapply( long.dat.grtr.10, long.dat$subjid, function(x) cumsum(cumsum(x)) ))
  last.visit.yes <- ifelse(visit.vec == 1, 1, 0)   # indicator for last visit before withdrawal
  visits.keep <- ifelse(visit.vec <= 1, 1, 0)      # indicator for visits to keep (remove others)
  
  ## Subset longitudinal data to remove visits after individuals withdraw due to high X.ijk values
  long.dat2 <- long.dat[ which(visits.keep == 1), ]
  max.visit <- tapply(long.dat2$time, long.dat2$subjid, max)   # max visit time per individual
  
  ## Update censored time to time of last visit for individuals who withdrew early for high X.ijk,
  ## and indicate which individuals experienced informative censoring
  surv.dat2 <- surv.dat
  surv.dat2$eventtime <- ifelse(surv.dat2$subjid %in% subj.inf.cens, max.visit,
                                surv.dat2$eventtime)
  surv.dat2$status <- ifelse(surv.dat2$subjid %in% subj.inf.cens, 0, surv.dat2$status)
  #surv.dat2$inf.cens <- ifelse(surv.dat2$subjid %in% subj.inf.cens, 1, 0)
  
  ## Return both datasets saved as a list
  data.list <- list(surv.dat = surv.dat2, long.dat = long.dat2)
  return(data.list)
